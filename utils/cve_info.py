...
def cvemap_search(cve, info_dir):
    ...
    try:
        # cvemap is an optional dependency; do not hard-fail the whole pipeline.
        subprocess.run(
            cvemap_command,
            shell=True,
            check=True,
        ...
        logging.error("cvemap binary not found in PATH. Skipping cvemap enrichment.")
        return None
    except subprocess.TimeoutExpired:
        logging.error(f"cvemap timeout for {cve}. Skipping.")
        return None
    except subprocess.CalledProcessError as e:
        logging.error(f"cvemap failed for {cve}: {e.stderr.strip()}")
        return None

    try:
        with open(cvemap_json_path, encoding="utf-8") as f:
            json_data = json.load(f)
        if not json_data:
            logging.error(f"Empty cvemap result for {cve}")
            return None
        return json_data[0]
    except Exception as e:
        logging.error(f"Failed to parse cvemap json for {cve}: {e}")
        return None
...
def searchsploit_search(cve):
    ...
    try:
        ...
        logging.error("searchsploit binary not found in PATH. Skipping ExploitDB lookup.")
        return {"RESULTS_EXPLOIT": []}
    except subprocess.TimeoutExpired:
        logging.error(f"searchsploit timeout for {cve}. Skipping.")
        return {"RESULTS_EXPLOIT": []}
    except subprocess.CalledProcessError as e:
    ...
    try:
        with open(searchsploit_json_path, encoding="utf-8") as f:
            return json.load(f)
    except Exception as e:
        logging.error(f"Failed to parse searchsploit json for {cve}: {e}")
        return {"RESULTS_EXPLOIT": []}
...
def get_final_score(func_score, comp_score):
    ...
    if func_score < 30 and comp_score < 15:
        multiplier = 0.2
    else:
        multiplier = 1
...
def calculate_score(features, trending_score):
    """
    Calculate CVE score, returning functionality score, complexity score, and final score
    """
    ...
    best_github = {'func': 0, 'comp': 0, 'final': 0, 'repo': None}
    best_expdb = {'func': 0, 'comp': 0, 'final': 0, 'repo': None}
    best_overall = {'func': 0, 'comp': 0, 'final': 0, 'repo': None}
    ...
    if features['code'].get('GitHub') or features['code'].get('ExploitDB'):
        for source in code_sources:
            ...
            has_code = True
            source_data = features['code'][source]
            
            for repo, vul_type in source_data["vul_type"].items():
                func_score = 0
                func_score += weights["vul_type"].get(vul_type, 0)
                func_score += weights["isRemote"].get(source_data["isRemote"].get(repo, ""), 0)
                
                if repo == "Code_File":
                    func_score = func_score / 2
                
                func_score *= weights["exp_maturity"].get(source_data["exp_maturity"].get(repo, ""), 0)
                
                comp_score = 0
                attack_complexity = source_data["attack_complexity"].get(repo, {})
                for field, value in attack_complexity.items():
                    comp_score += weights["attack_complexity"].get(field, {}).get(value, 0)
                
                comp_score *= weights["lang_class"].get(source_data["lang_class"].get(repo, ""), 1)
                
                comp_score *= weights.get("exp_flexibility", {}).get(source_data.get("exp_flexibility", {}).get(repo, ""), 1)
                
                merged_score = get_final_score(func_score, comp_score)
                
                source_weight = weights["source_weights"]["gthb"] if source == "GitHub" else weights["source_weights"]["expdb"]
                final_score = merged_score * source_weight
                ...
                is_expdb = repo.isdigit()
                best_current = best_expdb if is_expdb else best_github
                
                if final_score >= best_current['final']:
                    best_current.update({
                        'func': func_score,
                        'comp': comp_score,
                        'final': final_score,
        ...
        if best_expdb['final'] > best_github['final']:
            best_overall = best_expdb.copy()
        else:
            best_overall = best_github.copy()
        
        if has_code:
            sorted_scores = sorted(scores.items(), key=lambda item: item[1]['final'], reverse=True)
            ...
            if trending_score == 999:
                ...
                trending_score_expdb = weights["expdb_default_score"] if features['code'].get('ExploitDB') else 0
            
            trend_weighted_expdb = trending_score_expdb * weights["trending_score"]
            trend_weighted_github = trending_score_github * weights["trending_score"]
            
            final_expdb = (best_expdb['final'] + trend_weighted_expdb) if best_expdb['final'] > 0 else 0
            final_github = (best_github['final'] + trend_weighted_github) if best_github['final'] > 0 else 0
            
            if final_expdb > final_github:
                best_overall['final'] = final_expdb
                best_overall['repo'] = best_expdb['repo']
            else:
                best_overall['final'] = final_github
                best_overall['repo'] = best_github['repo']
            
            code_results = (sorted_scores, best_overall, has_code)
    
    if features['doc']:
        doc_data = features['doc']
        
        doc_func = 0
        doc_func += weights["vul_type"].get(doc_data["vul_type"], 0)
        doc_func += weights["isRemote"].get(doc_data["isRemote"], 0)
        
        doc_comp = 0
        for field, value in doc_data["attack_complexity"].items():
            doc_comp += weights["attack_complexity"].get(field, {}).get(value, 0)
        
        doc_merged = doc_func + 30 - doc_comp
        doc_final = doc_merged * weights["source_weights"]["gg"]
        
        scores["doc"] = {
            'functionality': doc_func,
            'complexity': doc_comp,
            'final': doc_final
        }
        
        if doc_final > best_overall['final']:
            best_overall.update({
                'func': doc_func,
                'comp': doc_comp,
                'final': doc_final,
        ...
        doc_results = (scores, best_overall, has_code)
    
    if code_results and doc_results:
        code_sorted, code_best, code_has = code_results
        doc_scores, doc_best, doc_has = doc_results
        
        all_scores = dict(code_sorted)
        all_scores.update(doc_scores)
        sorted_all = sorted(all_scores.items(), key=lambda item: item[1]['final'], reverse=True)
        
        final_best = doc_best if doc_best['final'] > code_best['final'] else code_best
        
        return sorted_all, final_best, code_has or doc_has
        ...
        return None, {'func': 0, 'comp': 0, 'final': 0, 'repo': None}, has_code
...
def cve_classifier(cve, output_dir="resources/", mode="specific"):
    ...
    trending_score = 999  # default score
    if features['code'].get('GitHub'):
        try:
    ...
    scores, best_result, has_code = calculate_score(features, trending_score)
    
    functionality_score = best_result['func']
    complexity_score = best_result['comp']
    final_score = best_result['final']
    max_score_repo = best_result['repo']
    ...
    return (cvss_score, cvss_category, epss_percentile, epss_category, 
            functionality_score, complexity_score, final_score, exploitability, has_code)
...
def analyze_cve_lst(cve_lst, output_dir, app_name):
    ...
    for cve in cve_lst:
        ...
        cvss_score, cvss_category, epss_score, epss_category, _, _, final_score, exploitability, has_code = cve_classifier(cve, output_dir, mode="specific")
