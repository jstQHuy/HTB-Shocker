...
def cvemap_search(cve, info_dir):
    ...
    try:
        ...
        logging.error("cvemap binary not found. Install cvemap or disable CVEMAP usage.")
        return None
    except subprocess.TimeoutExpired:
        logging.error(f"cvemap search timed out: {cve}")
        return None
    except subprocess.CalledProcessError as e:
        logging.error(f"cvemap search failed for {cve}: {e.stderr.strip()}")
        return None

    try:
        with open(cvemap_json_path, "r", encoding="utf-8") as f:
            json_data = json.load(f)
            if not json_data:
                logging.error(f"Empty cvemap result: {cve}")
                return None
            return json_data[0]
    except Exception as e:
        logging.error(f"Failed to read cvemap json for {cve}: {e}")
        return None
...
def normalize_data(df):
    """Normalize EEAS column to [0,1] safely.

    This function is called even when no CVE scoring data was collected.
    In that case df can be empty, and sklearn scalers would raise.
    """
    if df is None or len(df) == 0:
        return df
    if len(df) == 1:
        # For a single value, directly return 0.5 (neutral)
        df[['EEAS']] = 0.5
    else:
...
def analyze_cve_lst(cve_lst, output_dir, app_name):
    ...
    if num_df is None or getattr(num_df, "empty", False):
        logging.warning("No numeric score rows available for normalization; skipping score export/normalization.")
        normalized_num_df = num_df
        normalized_has_code_num_df = num_df
    else:
        normalized_num_df = normalize_data(num_df)
        normalized_has_code_num_df = normalized_num_df.loc[has_code_lst] if has_code_lst else normalized_num_df.head(0)
